...
class Rocket(object):
    ...
    def __init__(self, max_steps, task='hover', rocket_type='falcon',
        ...
        self.bg_img = load_bg_img(path_to_bg_img, w=self.viewport_w, h=self.viewport_h)
    ...
    def create_polygons(self):
        ...
        if self.rocket_type == 'falcon':
            ...
            # rocket paint
            pts = create_rectangle_poly(center=(0, -0.35*H), w=W, h=0.1*H)
            polys['rocket'].append({'pts': pts, 'face_color': (42, 42, 42), 'edge_color': None})
            pts = create_rectangle_poly(center=(0, -0.46*H), w=W, h=0.02*H)
            polys['rocket'].append({'pts': pts, 'face_color': (42, 42, 42), 'edge_color': None})
            # rocket landing rack
        ...
        if f > 0 and f < 0.5 * self.g:
            pts1 = create_rectangle_poly(center=(2 * dl * s, -H / 2 - 2 * dl * c), w=dl, h=dl)
            pts2 = create_rectangle_poly(center=(5 * dl * s, -H / 2 - 5 * dl * c), w=1.5 * dl, h=1.5 * dl)
            ...
            pts1 = create_rectangle_poly(center=(2 * dl * s, -H / 2 - 2 * dl * c), w=dl, h=dl)
            pts2 = create_rectangle_poly(center=(5 * dl * s, -H / 2 - 5 * dl * c), w=1.5 * dl, h=1.5 * dl)
            pts3 = create_rectangle_poly(center=(8 * dl * s, -H / 2 - 8 * dl * c), w=2 * dl, h=2 * dl)
            polys['engine_work'].append({'pts': pts1, 'face_color': (255, 255, 255), 'edge_color': None})
            polys['engine_work'].append({'pts': pts2, 'face_color': (255, 255, 255), 'edge_color': None})
            ...
            pts2 = create_rectangle_poly(center=(5 * dl * s, -H / 2 - 5 * dl * c), w=1.5 * dl, h=1.5 * dl)
            pts3 = create_rectangle_poly(center=(8 * dl * s, -H / 2 - 8 * dl * c), w=2 * dl, h=2 * dl)
            pts4 = create_rectangle_poly(center=(12 * dl * s, -H / 2 - 12 * dl * c), w=3 * dl, h=3 * dl)
            polys['engine_work'].append({'pts': pts1, 'face_color': (255, 255, 255), 'edge_color': None})
            polys['engine_work'].append({'pts': pts2, 'face_color': (255, 255, 255), 'edge_color': None})
            polys['engine_work'].append({'pts': pts3, 'face_color': (255, 255, 255), 'edge_color': None})
        ...
        # target region
        if self.task == 'hover':
            pts1 = create_rectangle_poly(center=(self.target_x, self.target_y), w=0, h=self.target_r/3.0)
            pts2 = create_rectangle_poly(center=(self.target_x, self.target_y), w=self.target_r/3.0, h=0)
            ...
            pts1 = create_ellipse_poly(center=(0, 0), rx=self.target_r, ry=self.target_r/4.0)
            pts2 = create_rectangle_poly(center=(0, 0), w=self.target_r/3.0, h=0)
            pts3 = create_rectangle_poly(center=(0, 0), w=0, h=self.target_r/6.0)
            polys['target_region'].append({'pts': pts1, 'face_color': None, 'edge_color': (242, 242, 242)})
            polys['target_region'].append({'pts': pts2, 'face_color': None, 'edge_color': (242, 242, 242)})
        ...
        for poly in polys['rocket'] + polys['engine_work']:
            M = create_pose_matrix(tx=self.state['x'], ty=self.state['y'], rz=self.state['theta'])
...
init_stdev = 2
#one agent per core
pop_size = 48 #CHANGE 48
m = MLP(8, 9)
num_params = sum([np.prod(param.shape) for param in m.parameters()])
seed = 1024
...
#parse arguments
parser = argparse.ArgumentParser(description="Handle flags in Python")
parser.add_argument('--output_folder', type=str, help='folder in which to store checkpoint files')
args = parser.parse_args()

output_folder = args.output_folder
task = 'hover'  # 'hover' or 'landing'

ckpt_folder = os.path.join(output_folder, task + '_ckpt')
...
def eval_param(params):
    ...
    for parameter in m.parameters():
        parameter.data = torch.tensor(params[s:s+np.prod(parameter.shape)].reshape(parameter.shape), dtype=torch.float32)
        s += np.prod(parameter.shape)
    state = env.reset()
    rewards, log_probs, masks = [], [], []
    for step_id in range(max_steps):
        action, log_prob = m.get_action(state)
...
with Pool(cpu_count()) as pool:
    for gen in range(num_gens):  # Main loop
        params = solver.ask()
        rewards = list(pool.map(eval_param, params))
        solver.tell(params, -1 * np.asarray(rewards))
        print(f'gen={gen}, best_reward={np.max(rewards)}, avg_reward={np.mean(rewards)}')
        if gen % 100 == 1:
            plt.figure()
            ...
            plt.savefig(os.path.join(ckpt_folder, 'best_rewards_' + str(gen).zfill(8) + '.jpg'))
            plt.close()
            plt.figure()
            plt.plot(AVG_REWARDS), plt.plot(moving_avg(AVG_REWARDS, N=50))
            plt.legend(['episode reward', 'moving avg'], loc=2)
            plt.xlabel('m episode')
            plt.ylabel('avg reward')
            plt.savefig(os.path.join(ckpt_folder, 'avg_rewards_' + str(gen).zfill(8) + '.jpg'))
            plt.close()
            best_params = params[np.argmax(rewards)]
            m = MLP(8, 9)
            s = 0
            for parameter in m.parameters():
                parameter.data = torch.tensor(best_params[s:s+np.prod(parameter.shape)].reshape(parameter.shape), dtype=torch.float32)
                s += np.prod(parameter.shape)
            torch.save({'episode_id': gen,
                        'REWARDS': REWARDS,
                        'model_G_state_dict': m.state_dict()},
                        os.path.join(ckpt_folder, 'ckpt_' + str(gen).zfill(8) + '.pt'))
        REWARDS.append(np.max(rewards))
